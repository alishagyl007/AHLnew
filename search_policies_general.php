    ...
    <div class="inner_section">
        <div class="row">
            <div class="col-md-4 important">
                <h3 class="page_title">Search by Vehicle No</h3>
                <div class="whitebg">
                    <form method="post">
...
$name=mysql_real_escape_string($_REQUEST['name']);
 $topic = mysql_query("select * from policies_general a,user b where a.user_id=b.user_id and a.vehicleno LIKE '%$name%'  order by a.id desc limit 10");
   
    }

if(isset($_REQUEST['btnmobile'])){
    $mobile=mysql_real_escape_string($_REQUEST['mobile']);
    $topic = mysql_query("select * from policies_general a,user b where a.user_id=b.user_id and b.mobile LIKE '%$mobile%'  order by a.id desc limit 10");
 
}

if(isset($_REQUEST['btndate'])){
    $sdate=mysql_real_escape_string($_REQUEST['sdate']);
    $topic = mysql_query("select * from policies_general a,user b where a.user_id=b.user_id and a.expire_date LIKE '%$sdate%' order by a.id desc limit 10");
}
